...
!***********************************************************************
      ...
      DIMENSION V(NAX,3,NSX),MAP(NAX,NSX,48,NP),S(3,3,48),NA(NSP)
...
!***********************************************************************
      ...
      DIMENSION MAGROT(48,NSAX),MAP(NAX,NSX,48,NP),S(3,3,48),NA(NSP)
...
!              located at the corresponding atomic position.           *
!                                                                      *
!      MAP(NAX,NSX,48,NP) contains a table connecting the rotated and  *
!                   the unrotated positions (stored is the index of    *
!                   the rotated position = first index in array TAU).  *
!                   MAP can be generated by routine CLASS (see below). *
!      S(3,3,48) contains the INTEGER rotation matrices.               *
...
!***********************************************************************
      ...
      DIMENSION F(3,NAX,NSX),MAP(NAX,NSX,48,NP),S(3,3,48),NA(NSP)
...
!***********************************************************************
      ...
      DIMENSION PT(NSAX+2,3),TR(NAX,3),MAP(NAX,NSX,48,NP)
...
! Perform the transformation (matrix multiplications):
         DO 101 II=1,3
          DO 101 JJ=1,3
           SUM=0._q
           DO 201 KK=1,3
 201       SUM=SUM+BDOTAI(II,KK)*R(KK,JJ)
 101     PROD(II,JJ)=SUM
         DO 102 II=1,3
          DO 102 JJ=1,3
            SUM=0._q
            DO 202 KK=1,3
 202        SUM=SUM+PROD(II,KK)*ADOTBI(KK,JJ)
 102     R(II,JJ)=SUM
