...
!       CF(M)=WDES%DATAKE(M,NK)*W%CPTWFP(M,N,NK,ISP)
        CF(M)=WDES%DATAKE(M,NK,ISP)*W%CPTWFP(M,N,NK,ISP)
...
! charge-density in real space:
      CALL FFT3RC(CHDEN(1,ISP),GRID_SOFT,1)
