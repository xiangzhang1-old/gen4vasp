    ...
    ltest1: IF (ltest) THEN
       IF (lop6) THEN
          ...
          SELECT CASE (nbmodus)
             ...
             CASE (-4,-5) ! nbmodus = -4 or -5: select bands via energy range
                WRITE(TIU6,'(A,I3)') "NBMOD is set to ",nbmodus
                WRITE(TIU6,'(A,2F9.4)') "Selected energy range (EINT) ",eint
                IF (sepmod==3) THEN
                  WRITE(TIU6,'(A,2F9.4)') "use triangular weights"
                ENDIF
          ...
          SELECT CASE (nbmodus)
             ...
             CASE (-4,-5) ! nbmodus = -4 or -5: select bands via energy range
                ...
                WRITE(TIU0,'(A,2F9.4)') "Selected energy range (EINT) ",eint
                IF (sepmod==3) THEN
                  WRITE(TIU0,'(A,2F9.4)') "Use triangular weights"
                ENDIF
...
! differnt, and the output to <<CHG>> is also differnt
        CALL OUTPOS(iuchgcar,.FALSE.,INFO%SZNAM1,LATT_CUR%SCALE,LATT_CUR%A,T_INFO%NTYP, &
            T_INFO%NITYP,.FALSE.,T_INFO%NIONS,DYN%POSION,T_INFO%LSFOR)

       CALL OUTCHG(GRIDC,iuchgcar,.TRUE.,CHTOT)

       IF (INFO%ISPIN==2) THEN
           WRITE(iuchgcar,'(5E20.12)') (T_INFO%ATOMOM(NI),NI=1,T_INFO%NIONS)
          CALL OUTCHG(GRIDC,iuchgcar,.TRUE.,CHTOT(1,2))
       ENDIF

        CLOSE(iuchgcar)

        CALL OUTPOS(iuchg,.FALSE.,INFO%SZNAM1,LATT_CUR%SCALE,LATT_CUR%A,T_INFO%NTYP, &
            T_INFO%NITYP,.FALSE.,T_INFO%NIONS,DYN%POSION,T_INFO%LSFOR)
       ...
       CASE (3)  sep    ! no seperation at all: merge everything
          ...
          kpoint3: DO ik=1,nkpoint
             ...
             DO ib=1,nband
                ...
                IF (nbmodus <= -4) THEN
                 DO isp=1,info%ispin
                   eigv = w%celtot(nb,nk,isp)
                   IF ((eint(1) <= eigv) .AND. (eigv <= eint(2))) THEN
                      eigv=(eigv -(eint(1)+eint(2))/2)/(eint(2)-eint(1))*2
                      w%fertot(nb,nk,isp) = 1-abs(eigv)
                   ELSE
                   ENDIF
                 ENDDO
                ELSE IF (nbmodus <= -2) THEN
                 DO isp=1,info%ispin
                   eigv = w%celtot(nb,nk,isp)
                   IF ((eint(1) <= eigv) .AND. (eigv <= eint(2))) THEN
                     w%fertot(nb,nk,isp) = 1.0
...
! first get status of the input file:
      ...
      ELSE  ! file is allready open
         ...
         IF (lop0) WRITE(TIU6,'(A,I3,1X,A,A)') "File ",iupar,filein," allready open."
      END IF
...
! try to find band index
      ...
      CASE (0)        ! IBAND found
         ...
         CASE(3) err_nbmod1
            IF(lop6) &
                 WRITE(TIU6,'(A)') "NBMOD not found, set to ",nbmodus
            IF(lop0) &
                 WRITE(TIU0,'(A)') "NBMOD not found, set to ",nbmodus
      ...
      CASE (3)        ! IBAND not found, search for other information
         ...
         CASE (0)  ! nbmod found
            ...
            CASE (-3,-5) nbmod      ! read eint vs. fermi energy
               nbmodus = idummy(1)
