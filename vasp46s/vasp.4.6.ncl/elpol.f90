...
!-----------------------------------------------------------------------
     ...
     &  P(NT)%PSDMAX,NPSRNL,LMYDIM,LYMAX,YLM(1,1),IRDMAX,INDMAX, &
...
!OCL NOVREC
         ...
         d_IJ(1:3,LM+M-1,LMP+MP-1,NI,ISP)=SUM*RINPL
         d_IJ(1:3,LMP+MP-1,LM+M-1,NI,ISP)=SUM*RINPL
!         WRITE(0,90) LL,M,LLP,MP,d_IJ(1:3,LM+M-1,LMP+MP-1,NI),TSUM*RINPL,P(NT)%QION(L,LP)
...
!=======================================================================
      ...
      CALL SETDEP(P(NT)%QDEP(1,1,1),P(NT)%PSDMAX,NPSRNL, &
     &        LATT_CUR%OMEGA,INDMAX,DIST(1),DEP(1))
...
!OCL NOVREC
      ...
      DO IND=1,3
         CALL CALC_DLLMM_BERRY( d_IJ(IND,:,:,NI,ISP),d_LM(IND,:),P(NT))
!         d_IJ(IND,:,:,NI,ISP)=d_IJ(:,:)
      ENDDO
      ENDIF lpaw
...
!***********************************************************************
     ...
     &     LMDIM, LOVERL, NBLOCK, CQIJ, IG, NK, NKP, ISP, CHAM, LIMAG)
...
!=======================================================================
      IF (LIMAG) THEN
         CSET_IN_SV=CMPLX(0._q,-1._q)
      ELSE
         CSET_IN_SV=CMPLX(1._q,0._q)
      ENDIF

      CALL SETWDES(WDES,WDES1,NK) ; CALL SETWGRID(WDES1,GRID)
      CALL SETWDES(WDES,WDES2,NKP); CALL SETWGRID(WDES2,GRID)
...
!***********************************************************************
     ...
     &  IGPAR,NPPSTR,P,T_INFO,LMDIM,CQIJ,IRDMAX,NBLOCK,LOVERL,IUNOUT,POSCEN)
...
!=======================================================================
      R_EV=0
      CALL SET_EV(WDES,W,ISP,LMDIM,d_IJ,LOVERL,R_EV)

      IF (IUNOUT>=0) THEN
         WRITE(IUNOUT,100) 'Expectation value term: <R>ev',R_EV(1:3)
      ENDIF
 100  FORMAT(/A/ &
     &      '<R> = ','(',F12.5,',',F12.5,',',F12.5,' ) electrons Angst',/)
...
!=======================================================================
     ...
     &     LMDIM, LOVERL, NBLOCK, CQIJ, IG, NK, NKP, ISP, CTMP, LIMAG)
         CMK_=CTMP
         CMK=0
         N=0;MN=0
...
!=======================================================================
         ...
         IF (IUNOUT>=0) THEN
            WRITE(IUNOUT,'(/A,I4,A,F8.4)')'K-point string # ',ISTR,' weight=', &
            WDES%WTKPT((ISTR-1)*NPPSTR+1)*NPPSTR
            DO IK=1,NPPSTR
               NTMP=KPTSTR(IK,ISTR)
               WRITE(IUNOUT,'(3F10.6)') KPOINTS%VKPT(1:3,NTMP)
            ENDDO
            WRITE(IUNOUT,'(A,F12.5,A)') 'Berry-Phase term: ',-BP/TPI,' electrons'
         ENDIF

         ! WDES%WTKPT((ISTR-1)*NPPSTR+1) is the weight of (1._q,0._q) string
         R_BP=R_BP+BP*(WDES%WTKPT((ISTR-1)*NPPSTR+1)*NPPSTR*WDES%RSPIN)
      ...
      IF (IUNOUT>=0) THEN
         WRITE(IUNOUT,130)'Berry-Phase term: <R>bp', &
        &                  R_BP*LATT_CUR%A(:,IGPAR),LATT_CUR%A(:,IGPAR)*WDES%RSPIN, &
        &                 'ionic term: <R>ion',DIP_ION
      ENDIF
 130  FORMAT(/A/ &
     &      '<R> = ','(',F12.5,',',F12.5,',',F12.5,' ) electrons Angst', / &
     &      '     +','(',F12.5,',',F12.5,',',F12.5,' ) *c', / &
...
!=======================================================================
! End of Berry-Phase calculation & Output of summary
!=======================================================================
...
!************************************************************************
      ...
      SUBROUTINE RD_KPOINTS_BERRY(KPOINTS,NPPSTR,IGPAR, &
           LATT_CUR, NKDIM,IKPTD,NTETD,LINVERSION,IU6,IU0)
      USE prec
      USE mkpoints
      USE lattice
...
! Internal rescaling and centering according to Monkhorst-Pack:
            IF ((CSEL=='M').OR.(CSEL=='m')) THEN
               SHIFT(1)=SHIFT(1)*FLOAT(NKPX)+0.5_q*MOD(NKPX+1,2)
               SHIFT(2)=SHIFT(2)*FLOAT(NKPY)+0.5_q*MOD(NKPY+1,2)
               SHIFT(3)=SHIFT(3)*FLOAT(NKPZ)+0.5_q*MOD(NKPZ+1,2)
            ENDIF
            DO I=1,3
               BK(I,1)=LATT_CUR%B(I,1)/FLOAT(NKPX)
               BK(I,2)=LATT_CUR%B(I,2)/FLOAT(NKPY)
