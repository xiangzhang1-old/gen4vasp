...
! caclulate Q |Mp>  Q (overlap-matrix) and store in CPRO0
      ...
      CALL OVERL(WDES1, LOVERL,LMDIM,CQIJ, CPROL,CPRO0)
! calculate the matrix U(N,M) = <N,Mp>
      NPRO=WDES%NPRO
      IF (.NOT.LOVERL) NPRO=0
      CALL ORTH2( &
     &  W%CPTWFP(1,1,NK,ISP),CPTWFL,W%CPROJ(1,1,NK,ISP),CPRO0,WDES%NBANDS,NBLK, &
...
! read in old wavefunction CPTWFL and rotate it by multiplying with CAP
     ...
     &  (W%CPTWFP(M,N,NK,ISP),M=1,NPL),(W%CPROJ(M,N,NK,ISP),M=1,WDES%NPRO)
      ...
      CALL LINCOM('F',W%CPTWFP(1,1,NK,ISP),W%CPROJ(1,1,NK,ISP),CAP, &
     &      WDES%NBANDS,WDES%NBANDS,NPL,WDES%NPRO,WDES%NRPLWV,WDES%NPROD,WDES%NBANDS, &
     &      NBLK,W%CPTWFP(1,1,NK,ISP),W%CPROJ(1,1,NK,ISP))
...
!***********************************************************************
      IF (ICALLS>=2) THEN
      CALL LINCOM('F',W%CPTWFP(1,1,NK,ISP),W%CPROJ(1,1,NK,ISP),CU, &
           WDES%NBANDS,WDES%NBANDS,NPL,WDES%NPRO,WDES%NRPLWV,WDES%NPROD,WDES%NBANDS, &
           NBLK,W%CPTWFP(1,1,NK,ISP),W%CPROJ(1,1,NK,ISP))
      ENDIF
!=======================================================================
