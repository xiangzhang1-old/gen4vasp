...
!***********************************************************************
      ...
      SUBROUTINE BRMIX(GRIDB,GRIDC,IO,MIX,B_TO_C, &
         NGIGA,CHTOT,CHTOTL,ISPIN,B,OMEGA, &
         N_MIX_PAW, RHOLM, RHOLM_LAST, &
         RMST,RMS,RMSP,WEIGHT,IERRBR)
      USE prec
...
! Set CHP (contains the metric to be used later):
      ...
      CALL BRPRE(GRIDB,CHP,B,MIX%MIXPRE,MIX%BMIX,MIX%LRESET.AND.(IDUMP/=0))
...
!OCL NOVREC
    ...
    ! set CHP = metric * (CHTOT-CHTOTL):
      ...
      ENDDO
!-----------------------------------------------------------------------
! Start mixing (given as input to BROYD on array CWRK1 if resent ...):
!-----------------------------------------------------------------------
      IF (MIX%LRESET) THEN
...
! Broyden mixing:
      ...
      CALL BROYD(NDATA,CWRK3,CWRK4,CHP,MIX%WC,IB,NUPDZ, &
                 IABS(MIX%MAXMIX),MIX%MAXMIX>0,MIX%MREMOVE,B,NGIGA, &
                 CWRK1,CWRK2,MIX%LRESET,MIX%IUBROY,IO%LOPEN,IO%ICMPLX,IO%MRECL, &
                 RMS,RMSP,WEIGHT,MIX%NEIG,MIX%EIGENVAL,MIX%AMEAN,IERR,GRIDC%COMM)
...
! Check whether charge density changed
      ...
      IF (ABS(TOTNEL-TOTOLD)>1E-5_q*TOTNEL) WRITE(*,11) TOTOLD,TOTNEL
...
!***********************************************************************
     ...
     &                  RMS,RMSP,WEIGHT,NEIG,EIGENVAL,AMEAN,IERR,COMM)
      USE prec
      USE mpimy
      ...
      REAL(q)    AUX(MAXIT,MAXIT),VV(MAXIT)
...
!=======================================================================
      ...
      IF (INFO/=0) THEN
       ...
       &        AUX,1,AUX,1,AUX,MAXIT*MAXIT,INFO)
...
! if IP=1 the inverse of a Hartree like matrix is used
! if IP=2 the inverse of a Kerker  like matrix is used
...
!
      TPI=8._q*ATAN(1._q)
      IF ((MIXPRE/=1).AND.(MIXPRE/=2).AND.(MIXPRE/=3)) THEN
! Unknown type: 'No metric':
         IF (LWRITE) WRITE(*,*) 'Routine BRPRE: No metric'
         P=1.0_q
      ELSE
         IF (MIXPRE==1 .OR. MIXPRE==2) THEN
