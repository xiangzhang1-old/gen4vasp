...
!***********************************************************************
      ...
      SUBROUTINE CALC_NABIJ(NABIJ,W,WDES,P,KPOINTS,GRID_SOFT,LATT_CUR, &
                            IO,INFO,T_INFO,NBLK,COMM,IU0,IU)
      USE prec
      USE base
      USE constant
      USE lattice
...
! get the matrix element of the nabla-operator in eV/Angstroem units
         CALL NABIJ_SOFT(NABIJ,IDIR,NK,ISP,W,WDES,GRID_SOFT,LATT_CUR,NBLK)
...
! write OPTIC
         ...
         IF (IU>=0) THEN
           CALL OUTOPT(NABIJ,IDIR,NK,ISP,W%CELTOT,W%FERTOT,NB_TOT, &
                       INFO%ISPIN,KPOINTS%NKPTS,NKDIM,WDES%VKPT, &
...
!***********************************************************************
      ...
      SUBROUTINE NABIJ_SOFT_(NABIJ,IDIR,NK,ISP,W,WDES,GRID,LATT_CUR,NBLOCK)
      USE prec
      USE constant
      USE lattice
      USE poscar
...
!***********************************************************************
      ...
      SUBROUTINE NABIJ_SOFT(NABIJ,IDIR,NK,ISP,W,WDES,GRID,LATT_CUR,NBLOCK)
      USE prec
      USE constant
      USE lattice
      USE poscar
      ...
      ALLOCATE(CBLOCK(NBLOCK,WDES%NB_TOT),GC(NBLOCK))
      ...
      block: DO IBLOCK=0,NPL-1,NBLOCK
       ...
       CALL ZGEMM('C', 'N', NB_TOT, NB_TOT,  ILENPL, (1._q,0._q), &
                  CBLOCK(1,1),  NBLOCK, W%CPTWFP(IADDPL+1,1,NK,ISP), &
...
! n_ij= < psi_i | nabla | psi_j > - < tilde psi_i | nable | tilde psi_j >
!
...
! subtraction can be done by subtraction of (1._q,0._q) from ONEBYR_SCALE (therefore we
...
! write out header of file OPTIC on I/O unit IU
      ...
      SUBROUTINE OUTOPT(NABIJ,IDIR,NK,ISP,CELEN,FERWE,NB_TOT,ISPIN, &
                        NKPTS,NKDIM,VKPT,WTKPT,IRECL,NBVAL,NBCON,NKOPT,NKOFF,IU)
      USE prec
      USE constant
      ...
      COMPLEX(q)   :: CELEN(NB_TOT,NKDIM,ISPIN)
      REAL(q)      :: VKPT(3,NKDIM),WTKPT(NKDIM),FERWE(NB_TOT,NKDIM,ISPIN)
