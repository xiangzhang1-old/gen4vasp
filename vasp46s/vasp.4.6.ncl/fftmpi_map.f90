...
! local variables
     ...
     C=> GRID%COMM
      ...
      IONODE =C%IONODE
!
! find maximum number of columns in each representation on all 
!
      MAXCOL_X=GRID%RC%NCOL
      MAXCOL_Y=GRID%IN%NCOL
      MAXCOL_Z=GRID%RL%NCOL
...
!
      ...
      ALLOCATE(GRID%IN_RL%TBL(NA2),  GRID%IN_RL%PTR(NC),  GRID%IN_RL%RMT(NC),  GRID%IN_RL%N(NC), &
               RCV_TBL2(NA2))
      NA3=GRID%RL%NALLOC     !  can receive onle number of RL-data
...
!
! copy the data distribution of node JNODE to all other 
!
...
!           copy own data to receive = send buffer
            ...
            END DO
            RECV%RL%NCOL = GRID%RL%NCOL
            DO ICOL_SND = 1,GRID%RL%NCOL
              RECV%RL%I2(ICOL_SND) = GRID%RL%I2(ICOL_SND)
              RECV%RL%I3(ICOL_SND) = GRID%RL%I3(ICOL_SND)
            END DO
        END IF

        CALL MPI_barrier( C%MPI_COMM, ierror )

        ! and now send it to all 
...
! the send and received data are continously stored on all  
...
! send positions, where data has to be stored (after global exchange), to remote 
      ...
      !    'map done',NODE_ME,I1,I3,I2,I4,GRID%RC%NP,GRID%IN%NP,GRID%RL%NP
...
!**********************************************************************
!
! this routine communicates the 3d fft data between  forward
...
!**********************************************************************
!
! this routine communicates the 3d fft data between  backward
...
!DIR$ IVDEP
      ...
      SUBROUTINE MAP_GATHER_ADD(N, INDEX, SRC, DEST)
      USE prec
      INTEGER N,INDEX(N)
      COMPLEX(q) SRC(*),DEST(*)
! local variables
