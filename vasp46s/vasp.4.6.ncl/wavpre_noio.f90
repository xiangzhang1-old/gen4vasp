...
!
! improved charge extrapolation added by Dario Alfe (Feb 1998)
!
!*********************************************************************

      SUBROUTINE WAVPRE_NOIO(GRIDC,P,PRED,T_INFO,W,WDES,LATT_CUR,LOPEN, &
         CHTOT,RHOLM,NPAW, CSTRF, LMDIM,CQIJ,LOVERL,NBLK,IU0)
      USE prec
      USE base
      USE poscar
      USE wave
...
!***********************************************************************
! PRED%INIPRE==1 or 3 ... performe initialisation of arrays
!***********************************************************************
...
!-----------------------------------------------------------------------
      ...
      CU=0
!  caclulate Q |cfin_n>
      CALL OVERL(WDES1, LOVERL,LMDIM,CQIJ,W%CPROJ(1,1,NK,ISP),CPROW(1,1))
...
! calculate the matrix U(N,M) = <N,Mp>
     ...
     &  NB_TOT,NBLK,1,NB_TOT,NPL,NPRO_O,NRPLWV_RED,NPROD_RED,CU(1,1))
      

      CALL ZGEMM( 'C', 'N', NB_TOT, NB_TOT, NB_TOT, (1._q,0._q), CU, NB_TOT, &
     &            CU, NB_TOT, (0._q,0._q), CAP, NB_TOT)
...
!
      ...
      ! the following line is required to make the code waterproof
...
! calculate AP(N,M)=A(N,K)*CONJG(U(M,K))
      CALL ZGEMM( 'N', 'C', NB_TOT, NB_TOT, NB_TOT, (1._q,0._q), CA, NB_TOT, &
     &            CU, NB_TOT, (0._q,0._q) , CAP, NB_TOT)
!-----------------------------------------------------------------------
...
! CU(N,M)=CONJG(CAP(K,N))*CA(K,M)
      ...
      CALL ZGEMM('C', 'N', NB_TOT, NB_TOT, NB_TOT, (1._q,0._q), CAP, NB_TOT, &
     &             CA, NB_TOT, (0._q,0._q) , CU, NB_TOT)
!=======================================================================
...
! rotate old wavefunction CPTWFP by multiplying with CAP
      CALL LINCOM('F',W_1%CPTWFP(1,1,NK,ISP),W_1%CPROJ(1,1,NK,ISP),CAP, &
     &      NB_TOT,NB_TOT, NPL,NPRO,NRPLWV_RED,NPROD_RED, NB_TOT, &
     &      NBLK,W_1%CPTWFP(1,1,NK,ISP),W_1%CPROJ(1,1,NK,ISP))

! rotate last change of wavefunction by CAP
      CALL LINCOM('F',W_2%CPTWFP(1,1,NK,ISP),W_2%CPROJ(1,1,NK,ISP),CAP, &
     &      NB_TOT,NB_TOT,NPL,NPRO,NRPLWV_RED,NPROD_RED, NB_TOT, &
     &      NBLK,W_2%CPTWFP(1,1,NK,ISP),W_2%CPROJ(1,1,NK,ISP))
