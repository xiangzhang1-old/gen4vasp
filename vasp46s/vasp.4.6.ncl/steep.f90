...
!----- local work arrays
      ...
      CALL NEWWAV(WSEARCH ,WDES,GRID%MPLWV*WDES%NRSPINORS,.TRUE.)
      CALL NEWWAV(WSEARCHL,WDES,GRID%MPLWV*WDES%NRSPINORS,.TRUE.)
!=======================================================================
!  INITIALISATION:
...
!-----------------------------------------------------------------------
      DO ISPINOR=0,WDES%NRSPINORS-1
        CALL FFTWAV(NGVECTOR,WDES1%NINDPW(1),W1%CR(1+ISPINOR*WDES1%MPLWV),W1%CPTWFP(1+ISPINOR*NGVECTOR),GRID)
      ENDDO
!-----------------------------------------------------------------------
...
!-MM- changes to accommodate spin spirals
! original statement
!         PRECON(MM)=1._q/(WDES1%DATAKE(M)+SLOCAL-EVALUE+ CMPLX( 0 , DE_ATT ,KIND=q) )
...
!-----------------------------------------------------------------------
     ...
     &    LMDIM,CDIJ(1,1,1,ISP),CQIJ(1,1,1,ISP), SV(1,ISP),WSEARCH%CPTWFP(1))
      DETEST=0
      FNORM=0
      DO M=1,WDES1%NPL
        CPT     =W1%CPTWFP(M)
...
!----------------------------------------------------------------------
      IF (INFO%LREAL) THEN
        DO ISPINOR=0,WDES%NRSPINORS-1
           CALL FFTWAV(NGVECTOR,WDES1%NINDPW(1),WSEARCH%CR(1+ISPINOR*WDES1%MPLWV),WSEARCH%CPTWFP(1+ISPINOR*NGVECTOR),GRID)
        ENDDO
      ...
      IF (INFO%LORTHO) THEN
        CALL ORTHON(WDES,NK,W,WSEARCH, INFO%LOVERL,LMDIM,CQIJ(1,1,1,ISP),ISP)
      ENDIF
!----------------------------------------------------------------------
! steepest descent  ok
!----------------------------------------------------------------------
...
!    search direction
!=======================================================================
      CALL PROJCN(WDES1,WSEARCH,W1, INFO%LOVERL,LMDIM,CQIJ(1,1,1,ISP), CSCPD)
      CALL CNORMN(WDES1,WSEARCH, INFO%LOVERL,LMDIM,CQIJ(1,1,1,ISP),WSCAL)
!----------------------------------------------------------------------
! panic here: our search direction ca not be normalised
